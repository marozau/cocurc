...
#ifndef _COCURC_BQUEUE_MTX_BQUEUE_H_
#define _COCURC_BQUEUE_MTX_BQUEUE_H_
...
namespace cocurc
{
	typedef std::mutex sync;
	typedef std::lock_guard< std::mutex > mtx;
}
#endif
...

	{
...

		{
...

			{
				...
				static const int padding_size = BOOST_LOCKFREE_CACHELINE_BYTES - sizeof( size_t ) - sizeof( cocurc::sync );

				cocurc::sync mtx_bqueue_protector_;
				...
				BOOST_DELETED_FUNCTION( mtx_bringbuffer_base( mtx_bringbuffer_base const& ) )

					BOOST_DELETED_FUNCTION( mtx_bringbuffer_base& operator= (mtx_bringbuffer_base const&) )



			protected:

				mtx_bringbuffer_base( void ) :
				...
				{
					cocurc::mtx mutex( mtx_bqueue_protector_ );
			...
			class mtx_bcompile_time_sized_ringbuffer :

				public mtx_bringbuffer_base < T >
			...
			public:
				...
				mtx_bcompile_time_sized_ringbuffer()
			...
			public:
...

				{
					...
					return mtx_bringbuffer_base<T>::push( f, data(), max_size );
...

				{
					...
					return mtx_bringbuffer_base<T>::consume_one( f, data(), max_size );
...

				{
					...
					return mtx_bringbuffer_base<T>::consume_all( f, data(), max_size ) != 0;
...
namespace cocurc
{
	template <typename T, std::size_t MaxSize> using mtx_bqueue = boost::lockfree::detail::mtx_bcompile_time_sized_ringbuffer< T, MaxSize >;
